...
2. **Proxy + 惰性实体化**:引擎层资源(`GfxTexture`/`GfxBuffer` 等)是轻量 Proxy,持有 `desc` 与 `m_gpu_ready` 标志;真正的 GPU 对象(`cutie::TextureHandle`)在 `initializeGpu()` 时才创建。这使"任意线程创建资源、渲染线程统一实体化"成为可能。
...
| [gbuffer-shaders.md](gbuffer-shaders.md) | GBuffer Shader 逐模块说明:顶点变换、GPU-driven、Bindless/Non-bindless MRT、全屏 DeferredLight、PBR/IBL/阴影以及 TAA motion vector 接线 |
