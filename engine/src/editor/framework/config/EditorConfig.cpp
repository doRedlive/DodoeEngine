...
bool EditorConfig::load(const std::string& builtinDir, const std::string& projectDir, const std::string& userDir)
{
    ...
    if (m_editor.is_null()) {
        LOG_ERROR("[EditorConfig] Failed to load editor.json from {}", builtinDir);
    ...
    LOG_INFO("[EditorConfig] Loaded theme={}, defaultLayout={}", themeName(), defaultLayoutName());
    return true;
}
...
dodoe::Json EditorConfig::loadJsonFile(const std::string& path) const
{
    ...
    if (!file.is_open()) {
        LOG_WARN("[EditorConfig] Cannot open: {}", path);
        return dodoe::Json();
    }
    ...
    try {
        ...
        LOG_WARN("[EditorConfig] Parse error {}: {}", path, e.what());
        return dodoe::Json();
    }
}
