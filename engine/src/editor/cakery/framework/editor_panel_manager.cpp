...
namespace cakery {
	...
	void EditorPanelManager::registerPanel(Scope<EditorPanel> panel) {
		if (!panel) {
			return;
		}

		const String id = panel->getDescriptor().id;
	...
	void EditorPanelManager::drawViewMenuItems() {
		...
		UnorderedMap<String, DynamicArray<EditorPanel*>> by_category{};
		DynamicArray<String> category_order{};
		...
		for (const auto& panel : m_panels) {
			...
			String category = descriptor.category.empty() ? "Panels" : descriptor.category;
	...
	EditorPanel* EditorPanelManager::findPanel(const String& id) const {
	...
	void EditorPanelManager::sortPanels() {
		std::ranges::sort(m_panels, [](const Scope<EditorPanel>& lhs, const Scope<EditorPanel>& rhs) {
	...
	void EditorPanelManager::applyDefaultLayout(const EditorPanelStage stage) {
		...
		// Convert Scope<> vector to raw pointer vector for layout builder
		DynamicArray<const EditorPanel*> panels_raw;
		panels_raw.reserve(m_panels.size());
		for (const auto& panel : m_panels) {
			panels_raw.push_back(panel.get());
		}

		m_layout_builder.buildDefaultLayout(m_dockspace_id, viewport, panels_raw, stage);
