...
namespace dodoe {
    ...
    namespace {
        ...
        UnorderedMap<InstanceID, Scope<AnimatorController>> s_animator_controller_cache{};
    ...
    Bool AnimatorController::loadFromJson(const String& absolute_path) {
    ...
    Bool AnimatorController::saveToJson(const String& absolute_path) const {
    ...
    AnimatorController* AnimatorController::Create(const ObjectID& ref, const String& path) {
        ...
        if (const InstanceID existing = Object::FindInstanceID(ref); existing != 0) {
            if (auto* obj = Object::FindObjectFromInstanceID(existing)) {
                return static_cast<AnimatorController*>(obj);
            }
        }
        ...
        auto controller = create_scope<AnimatorController>(ref);
        AnimatorController* raw = controller.get();
        raw->setPath(absolute_path);
        raw->loadFromJson(absolute_path);
        const InstanceID instance_id = raw->getInstanceID();
        s_animator_controller_cache.emplace(instance_id, std::move(controller));
        return raw;
    }

    void AnimatorController::Shutdown() {
        s_animator_controller_cache.clear();
    }
