...
namespace dodoe {
	...
	void FXAAPass::execute(size_t index) {
		(void)index;
		...
		m_cmd_list->open();
		m_cmd_list->beginMarker("FXAAPass");
	...
	void FXAAPass::onViewportResize(const Vector2i& viewport_extent) {
	...
	void FXAAPass::refreshInputResources() {
	...
	void FXAAPass::createShaders() {
		auto vert_source = ReadShaderFile("engine/res/shaders/bin/fullscreen.vert.spv");
		auto frag_source = ReadShaderFile("engine/res/shaders/bin/fxaa_pass.frag.spv");
		m_vertex_shader = m_rhi->getDevice()->createShader(
			rhi::ShaderDesc().setShaderType(rhi::ShaderType::Vertex).setEntryName("main").setDebugName("FXAAPass VS"),
			vert_source.data(), vert_source.size());
		m_pixel_shader = m_rhi->getDevice()->createShader(
			rhi::ShaderDesc().setShaderType(rhi::ShaderType::Pixel).setEntryName("main").setDebugName("FXAAPass PS"),
			frag_source.data(), frag_source.size());
	}

	void FXAAPass::createSampler() {
		m_sampler = m_rhi->getDevice()->createSampler(rhi::SamplerDesc());
	}

	void FXAAPass::createBindingLayout() {
	...
	void FXAAPass::createBindingSet() {
	...
	void FXAAPass::createFramebuffer() {
		refreshInputResources();
	...
	void FXAAPass::createGraphicsPipeline() {
