...
namespace dodoe {
    ...
    namespace {
        constexpr UInt64 kOpaquePassConstantBufferSize = 272;
    }

    struct OpaquePassConstantBuffer {
    ...
    static_assert(sizeof(OpaquePassConstantBuffer) <= kOpaquePassConstantBufferSize);

    static void BuildOpaquePassConstantBuffer(OpaquePassConstantBuffer& pass_cb,
    ...
    static GfxBindingSetHandle CreateOpaquePassBindingSet(DrawCommandList& command_list,
    ...
    static GfxBindingLayoutHandle MakeOpaquePassBindingLayout(BindingLayoutCache& binding_layout_cache) {
    ...
    void OpaquePass::build(RenderGraphBuilder& graph,
        ...
        const auto binding_layout = MakeOpaquePassBindingLayout(*binding_layout_cache);
        ...
        graph.addPass<OpaquePassParameters>(
            ...
            (RenderGraphPassBuilder& pass_builder, OpaquePassParameters& parameters) {
                ...
                parameters.depth = pass_builder.writeDepth(pass_builder.createTransientTexture(
                    rendering_pipeline_utils::MakeSwapchainRT2D(swapchain_extent, GfxFormat::D32, "RDG MainCameraDepth"),
                    "MainCameraDepth"), depth_attachment);
            ...
            [this, binding_layout](const OpaquePassParameters& parameters,
                ...
                OpaquePassConstantBuffer pass_cb{};
                BuildOpaquePassConstantBuffer(pass_cb, *ctx.getScene(), camera_position);
                ...
                const auto allocation = staging->allocate(kOpaquePassConstantBufferSize);
                ...
                const auto pass_binding_set = CreateOpaquePassBindingSet(
    ...
    void TransparentPass::build(RenderGraphBuilder& graph,
        ...
        graph.addPass<TransparentPassParameters>(
            ...
            [this, binding_layout](const TransparentPassParameters& parameters,
                ...
                auto* feature = static_cast<TransparentSceneFeature*>(m_owning_feature);
                ...
                const auto fb = ctx.getFramebuffer();
                const auto& draw_list = feature->getTransparentDrawLists()[ctx.getViewIndex()];
                SubmitMeshDrawCommands(draw_list.cached_instances, *draw_list.cached_commands,
                    draw_list.cached_shader_data, processor->getPrimitiveConstantBuffer(),
                    fb, viewport_state, resolved_psb, &pass_binding_set, command_list);
                SubmitMeshDrawCommands(draw_list.dynamic_instances, draw_list.frame_commands,
