...
public:
    UIDraggablePanel(Context& context,
                     Vector2f position,
                     Vector2f size,
    ...
    void setSkinImage(Image image) { if (content_panel_ptr_) content_panel_ptr_->setSkinImage(std::move(image)); }
    void setNineSliceMargins(std::optional<NineSliceMargins> margins) { if (content_panel_ptr_) content_panel_ptr_->setNineSliceMargins(std::move(margins)); }
    ...
    const NineSliceMargins* getNineSliceMargins() const { return content_panel_ptr_ ? content_panel_ptr_->getNineSliceMargins() : nullptr; }
